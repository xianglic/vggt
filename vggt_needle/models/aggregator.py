...
class Aggregator(nn.Module):
    ...
    def __init__(
        ...
        self.camera_token = nn.Parameter(init.randn(1, 2, 1, embed_dim))
        self.register_token = nn.Parameter(init.randn(1, 2, num_register_tokens, embed_dim))
    ...
    def forward(self, images: Tensor) -> Tuple[List[Tensor], int]:
        ...
        tokens = ops.cat([camera_token, register_token, patch_tokens], dim=1)
        ...
        if self.patch_start_idx > 0:
            ...
            pos = pos + 1
            pos_special = init.zeros(B * S, self.patch_start_idx, 2, device=images.device, dtype=pos.dtype)
            pos = ops.cat([pos_special, pos], dim=1)
        ...
        for _ in range(self.aa_block_num):
            ...
            for i in range(len(frame_intermediates)):
                ...
                concat_inter = ops.cat([frame_intermediates[i], global_intermediates[i]], dim=-1)
...
def slice_expand_and_flatten(token_tensor, B, S):
    ...
    query = token_tensor[:, 0:1, :, :].broadcast_to((B, 1, *token_tensor.shape[2:]))
    # Slice out the "other" tokens => shape (1, S-1, ...)
    others = token_tensor[:, 1:, :, :].broadcast_to((B, S - 1, *token_tensor.shape[2:]))
    # Concatenate => shape (B, S, ...)
    combined = ops.cat([query, others], dim=1)
