...
def run_model(target_dir, model) -> dict:
    """
    Run the VGGT_needle model on images in the 'target_dir/images' folder and return predictions.
    Needle is used for the forward pass, but we convert results back to torch so that
    pose_encoding_to_extri_intri and unproject_depth_map_to_point_map still work unchanged.
    """
    print(f"Processing images from {target_dir}")

    # --- Device for torch-side utilities (pre/post) ---
    device = "cuda" if torch.cuda.is_available() else "cpu"
    if not torch.cuda.is_available():
        raise ValueError("CUDA is not available. Check your environment.")

    # Make sure the Needle model is on the right device (string "cuda" is OK for Needle)
    model = model.to(device)
    model.eval()

    # --- Load and preprocess images with the original torch loader ---
    ...
    images_torch = load_and_preprocess_images(image_names).to(device)
    print(f"Preprocessed images shape (torch): {images_torch.shape}")
    ...
    print("Running inference with VGGT_needle...")
    # No torch.autocast here; the model is pure Needle.
    print(type(model.aggregator.patch_embed.patch_embed.proj.weight))
    print(model.aggregator.patch_embed.patch_embed.proj.weight.shape)
    predictions_needle = model(images_needle)

    # --- Convert Needle predictions -> torch tensors for downstream utilities ---
    predictions: dict = {}
    for key, value in predictions_needle.items():
        if isinstance(value, needle.Tensor):
            nd = value.realize_cached_data()      # NDArray
            np_val = nd.numpy()                  # numpy array
            predictions[key] = torch.from_numpy(np_val).to(device)
        else:
    ...
    extrinsic, intrinsic = pose_encoding_to_extri_intri(
        predictions["pose_enc"], images_torch.shape[-2:]
    )
    predictions["extrinsic"] = extrinsic
    predictions["intrinsic"] = intrinsic

    # --- Convert tensors to numpy (same as original code) ---
    ...
    # --- Generate world points from depth map (numpy/tensor utils) ---
    print("Computing world points from depth map...")
    depth_map = predictions["depth"]  # (S, H, W, 1) after the squeeze(0)
    world_points = unproject_depth_map_to_point_map(
        depth_map, predictions["extrinsic"], predictions["intrinsic"]
    )
    predictions["world_points_from_depth"] = world_points

    # Clean up torch CUDA mem
